...
class Department(NestedSet):
	...
	def create_user_and_employee(self, email, full_name):
		...
		if employee:
			...
			return {
				"redirect_to": f"/app/employee/{employee}?full_name={full_name}&department={self.name}"
			}

		# 2. Mở form Employee với các trường mặc định
		return {
			"redirect_to": f"/app/employee/new-employee-1?full_name={full_name}&department={self.name}&user_id={email}",
		}
